...
std::vector<std::string>Automata::etMenu() {
	...
	for (int i = 0; i < menu.size(); i++) fullMenu.push_back(menu[i] + std::to_string(prices[i]));		
	return fullMenu;
}

bool Automata::check(int choice) {
	if (state == CHECK) {
		if (cash - prices[choice] >= 0)	return true;
	}
	return false;
}
...
void Automata::choice(int choice) {
	if (getState() == ACCEPT) {
		state = CHECK;
		if (choice < menu.size() && choice >= 0 && check(choice)) {
			cash -= prices[choice];
			cook();
		}
		else cancel();
