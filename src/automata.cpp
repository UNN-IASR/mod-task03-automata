...
void CoffeeMachine::create_engine() {
	...
	transitions[std::pair<State, Act>(State::WAIT, Act::OFFACT)] = State::OFF;
	...
	transitions[std::pair<State, Act>(State::ACCEPT, Act::FALSECHECK)] = State::ACCEPT;
...
void CoffeeMachine::choice(Product product) {
	if (cash < menu[product]) {
		printf("\nNot enough money for chosen product. You need %s", std::to_string(menu[product] - cash).c_str());
...
Automata<State, Act>::Automata(std::map<std::pair<State, Act>, State> transitions, State start_state) {
...
void Automata<State, Act>::do_transition(Act act) {
	if (this->transitions.find(std::pair<State, Act>(this->getState(), act)) == this->transitions.end())
	{
		throw std::invalid_argument("Recieved wrong trasition call");
	}
	else {
		this->current_state = transitions[std::pair<State, Act>(this->getState(), act)];
	}
}
